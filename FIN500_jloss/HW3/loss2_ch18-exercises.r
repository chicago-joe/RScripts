...
fxspot.raw <- Quandl(fxspot.tickers[1], type = "xts")[, "Rate"]
for (i in 2:length(fxspot.tickers)) {
  fxspot.temp <- Quandl(fxspot.tickers[i], type = "xts")[, "Rate"]
...
fxfut.raw <- Quandl(fxfut.tickers[1], type = "xts")[, "Settle"]
for (i in 2:length(fxfut.tickers)) {
  fxfut.temp <- Quandl(fxfut.tickers[i], type = "xts")[, "Settle"]
...
alldata.full <- alldata.full["2014/2018-01-01"]
alldata.full<-na.locf(alldata.full)

names<-colnames(alldata.full)

# here some calculations have to happen...
for (i in 1:alldata.full)
 {
 tmp<-1000*(1+alldata.full$X3MUSDLIBOR+1)*alldata.full$USDEUR.spot
 }
