...
etf.rets <- diff(log(etf.prices))["2014/2018-01-01"]
...
commodity.rets<-diff(log(commodity.prices))["2014/2018-01-01"]

all.returns.tmp <- diff(log(cbind(etf.prices,commodity.prices)))["2014/2018-01-01"]
...
eff.frontier <- create.EfficientFrontier(R=all.returns, portfolio=port.spec,n.portfolios=100, type="mean-StdDev")
...
etf.returns.tmp <- diff(log(etf.prices))["2014/2018-01-01"]
etf.returns <- na.omit(etf.returns.tmp)

## set up portfolio with objective and constraints
n.assets <- length(colnames(etf.returns))
port.spec <- portfolio.spec(assets = colnames(etf.returns))
port.spec <- add.objective(portfolio=port.spec, type="risk", name="StdDev")
port.spec <- add.objective(portfolio=port.spec, type="return", name="mean")
port.spec <- add.constraint(portfolio=port.spec, type="full_investment")
port.spec <- add.constraint(portfolio=port.spec, type="box", min=-1, max=1)
...
## map out the efficient frontier (for variance risk)
eff.frontier <- create.EfficientFrontier(R=etf.returns, portfolio=port.spec,
...
comm.returns.tmp <- diff(log(commodity.prices))["2014/2018-01-01"]
comm.returns <- na.omit(comm.returns.tmp)

## set up portfolio with objective and constraints
n.assets <- length(colnames(comm.returns))
port.spec <- portfolio.spec(assets = colnames(comm.returns))
port.spec <- add.objective(portfolio=port.spec, type="risk", name="StdDev")
port.spec <- add.objective(portfolio=port.spec, type="return", name="mean")
port.spec <- add.constraint(portfolio=port.spec, type="full_investment")
port.spec <- add.constraint(portfolio=port.spec, type="box", min=-1, max=1)
...
## map out the efficient frontier (for variance risk)
eff.frontier <- create.EfficientFrontier(R=comm.returns, portfolio=port.spec,
                                         n.portfolios=100, type="mean-StdDev")
