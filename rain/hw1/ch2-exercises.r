...
Quandl.api_key("8S-D5kVjUxoxhLUmPHCQ")
...
for (j in 2:length(re.tickers)) {#error can't download
...
summary(alldata)
summary(ust.raw)
boxplot(as.matrix(ust.raw), col = "orange")

summary(corpbond.raw) 
boxplot(as.matrix(corpbond.raw), col = "gray")

summary(re.raw)
boxplot(as.matrix(re.raw), col = "red")

summary(commodities.raw)
boxplot(as.matrix(commodities.raw), col = "yellow")

summary(eqtidx.raw)
boxplot(as.matrix(eqtidx.raw), col = "green")

summary(fx.raw)
boxplot(as.matrix(fx.raw), col = "blue")
